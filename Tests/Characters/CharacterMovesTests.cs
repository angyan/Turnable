...
public class CharacterMovesTests
{
    ...
    internal void A_character_can_be_moved_to_a_new_location()
    {
        ...
        CharacterLocations initialCharacterLocations = new();
        initialCharacterLocations =
            initialCharacterLocations.AddOnMap(character, new Location(1, 1), map,
                new CollisionMasks(new[] { 1 }));
        initialCharacterLocations =
        ...
        sut = sut.Move(character, new Location(2, 1), map,
            new CollisionMasks(new[] { 1 }));

        ImmutableList<Location> character1Moves = sut.Value[character];
    ...
    internal void A_character_that_is_not_located_is_added_with_an_initial_location_when_moved()
    {
        ...
        sut = sut.Move(character, new Location(1, 1), map,
            new CollisionMasks(new[] { 1 }));
    ...
    internal void A_character_cannot_be_moved_to_its_own_location()
    {
        ...
        sut.Invoking(s => s.Move(character, new Location(1, 1), map, new CollisionMasks(new[] { 1 })))
            .Should().Throw<ArgumentException>().WithMessage($"Character '{character.Name}' cannot be moved to {location} because it is already there");
    }
    ...
    internal void A_character_cannot_be_moved_to_another_characters_location()
    {
        ...
        sut.Invoking(s => s.Move(character, location, map, new CollisionMasks(new[] { 1 })))
            .Should().Throw<ArgumentException>().WithMessage($"Character '{character.Name}' cannot be added at {location} because another character is already at {location}");
    }
    ...
    internal void A_character_cannot_be_moved_to_an_obstacles_location()
    {
        ...
        sut.Invoking(s => s.Move(character, location, map, new CollisionMasks(new[] { 1 })))
            .Should().Throw<ArgumentException>().WithMessage($"Character '{character.Name}' cannot be moved to {location} because there is an obstacle there");
    }
    ...
    internal void The_current_location_of_each_character_is_the_last_location_that__the_character_moved_to()
    {
        ...
        sut = sut.Move(character, new Location(2, 1), map, new CollisionMasks(new[] { 1 }));
        sut = sut.Move(character2, new Location(4, 1), map, new CollisionMasks(new[] { 1 }));
        sut = sut.Move(character, new Location(2, 3), map, new CollisionMasks(new[] { 1 }));
        ImmutableDictionary<Character, Location> currentLocations = sut.GetLocations();

        currentLocations.Count.Should().Be(2);
        currentLocations[character].Should().Be(new Location(2, 3));
    ...
    internal void The_moves_for_a_character_are_in_reverse_order()
    {
        ...
        sut = sut.Move(character, new Location(2, 1), map, new CollisionMasks(new[] { 1 }));
        sut = sut.Move(character, new Location(2, 3), map, new CollisionMasks(new[] { 1 }));
        ImmutableList<Location> character1Moves = sut.GetMoves(character);
