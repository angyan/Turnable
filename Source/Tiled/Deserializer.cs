...
namespace Turnable.Tiled
{
    internal static class Deserializer
    {
        ...
        internal static Map Deserialize(this MapJsonString mapJsonString) => JsonSerializer.Deserialize<Map>(mapJsonString.Value, JsonSerializerOptions)!;

        // NOTE: TilesetJsonString can never have the value "null" which is the only JSON string that returns null when deserialized
        internal static Tileset Deserialize(this TilesetJsonString tilesetJsonString) => JsonSerializer.Deserialize<Tileset>(tilesetJsonString.Value, JsonSerializerOptions)!;

        internal static Tileset DeserializeAndMerge(this Tileset tileset, TilesetJsonString tilesetJsonString)
        {
            Tileset deserializedTileset = JsonSerializer.Deserialize<Tileset>(tilesetJsonString.Value, JsonSerializerOptions)!;
